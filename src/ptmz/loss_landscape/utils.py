...
def compute_train_eval_loss(
    model, traineval_loader, criterion, use_regularization=False, regularizer=None
):
    ...
    for batch_idx, (inputs, targets) in enumerate(traineval_loader):
        inputs, targets = inputs.cuda(), targets.cuda()
        outputs = model(inputs)
        if use_regularization:
            loss = criterion(outputs, targets).item() + regularizer(model)
            total_loss += loss * targets.size()[0]
        else:
            total_loss += criterion(outputs, targets).item() * targets.size()[0]
        total_num += len(inputs)

    train_eval_loss = total_loss / total_num
...
def exp_lr_scheduler_layerwise(
    epoch, optimizer, strategy=True, decay_eff=0.1, decayEpoch=[]
):
    """Decay learning rate by a factor of lr_decay every lr_decay_epoch epochs"""
    print(strategy)
...
def fb_warmup(optimizer, epoch, baselr, large_ratio):
    for param_group in optimizer.param_groups:
        param_group["lr"] = epoch * (baselr * large_ratio - baselr) / 5.0 + baselr
    return optimizer
...
def _get_weights(model):
    ...
    W = [
        ...
        if (
            is_substring(["conv", "fc", "linear", "classifier"], name)
            and ("weight" in name)
        )
    ]
...
def get_margin(model, loader, num_batches=10, if_quantile=False):
    global diff, row, i, idx
    margins = []
    ...
    print("Showing the margin tensor size")
    print(margins.size())
    margins = margins.cpu().data.numpy()

    ave_margin = np.mean(margins)
    min_margin = np.min(margins)

    num_data = margins.shape[0]
    print(num_data)

    quantile_margin = np.sort(margins)[int(num_data / 20)]

    print(
        "The average margin is {0} and the min margin is {1}".format(
            ave_margin, min_margin
        )
    )
    ...
    if not if_quantile:
        ...
        return margins, ave_margin, min_margin, quantile_margin


"""
def return_two_models(file_name1, file_name2, arch = 'resnet20'):
    
    checkpoint1 = torch.load(file_name1)
    checkpoint2 = torch.load(file_name2)
    
    if arch == 'resnet20':  
        model1 = resnet20_cifar().cuda()
        model2 = resnet20_cifar().cuda()
    elif 'ResNet18' in arch:
        model1 = ResNet18(**arch_kwargs).cuda()
        model2 = ResNet18(**arch_kwargs).cuda()
    elif arch == 'VGG11':
        model1 = VGG('VGG11').cuda()
        model2 = VGG('VGG11').cuda()
...
def return_file_name(args, exp_id1, exp_id2):
    ...
    early_stopped_checkpoint1 = os.path.join(
        args.checkpoint_folder, f"net_exp_{exp_id1}_early_stopped_model.pkl"
    )
    early_stopped_checkpoint2 = os.path.join(
        args.checkpoint_folder, f"net_exp_{exp_id2}_early_stopped_model.pkl"
    )
