...
class BottleneckCurve(nn.Module):
    ...
    def __init__(self, in_planes, planes, fix_points, stride=1):
        ...
        self.conv1 = curves.Conv2d(
            in_planes, planes, fix_points=fix_points, kernel_size=1, bias=False
        )
        self.bn1 = curves.BatchNorm2d(planes, fix_points=fix_points)
        self.conv2 = curves.Conv2d(
            planes,
...
class ResNetCurve(nn.Module):
    def __init__(self, block, num_blocks, fix_points, num_classes=10, k=1):
        ...
        self.layer1 = self._make_layer(
            block, 1 * k, num_blocks[0], stride=1, fix_points=fix_points
        )
        self.layer2 = self._make_layer(
            block, 2 * k, num_blocks[1], stride=2, fix_points=fix_points
        )
        self.layer3 = self._make_layer(
            block, 4 * k, num_blocks[2], stride=2, fix_points=fix_points
        )
        self.layer4 = self._make_layer(
            block, 8 * k, num_blocks[3], stride=2, fix_points=fix_points
        )
        self.linear = curves.Linear(
            8 * k * block.expansion, num_classes, fix_points=fix_points
        )
...
def ResNet34Curve(num_classes, fix_points, width=1):
    return ResNetCurve(
        BasicBlockCurve,
        [3, 4, 6, 3],
        fix_points=fix_points,
...
def ResNet50Curve(num_classes, fix_points, width=1):
    return ResNetCurve(
        BottleneckCurve,
        [3, 4, 6, 3],
        fix_points=fix_points,
...
def ResNet101Curve(num_classes, fix_points, width=1):
    return ResNetCurve(
        BottleneckCurve,
        [3, 4, 23, 3],
        fix_points=fix_points,
...
def ResNet152Curve(num_classes, fix_points, width=1):
    return ResNetCurve(
        BottleneckCurve,
        [3, 8, 36, 3],
        fix_points=fix_points,
