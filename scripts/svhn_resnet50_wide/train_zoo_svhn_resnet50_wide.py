...
parser.add_argument(
    "--path-root", type=str, default="/ds2/model_zoos/taxonomy/svhn/", help=""
)
parser.add_argument(
    "--data-path",
    type=str,
    default="/ds2/model_zoos/taxonomy/svhn/",
    help="",
)
...
def main():
    ...
    # experiment name
    experiment_name = f"tune_zoo_SVHN_{model_type}_kaiming_uniform_{NUM_EPOCH}eps_width_{width_list[0]}-{width_list[-1]}"
    ...
    net_dir = PATH_ROOT.joinpath(f"zoos/SVHN/{config['model::type']}_v2")
    try:
    ...
    if not os.path.exists(data_path.joinpath("dataset_preprocessed.pt")):
        svhn_path = data_path.joinpath("image_data/SVHN")
        train_transforms = transforms.Compose(
            [
                ...
                transforms.Normalize(
                    mean=[0.4377, 0.4438, 0.4728], std=[0.198, 0.201, 0.197]
                ),
            ]
        )
