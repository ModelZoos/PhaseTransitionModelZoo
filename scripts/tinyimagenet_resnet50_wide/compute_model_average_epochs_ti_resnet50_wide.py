...
PATH_ROOT = Path(
    "/ds2/model_zoos/taxonomy/tiny_imagenet/zoos/TinyImageNet/Resnet50_width_v2"
)
...
def main():
    ...
    # experiment name
    experiment_name = f"model_average_epochs_TinyImagenet_{model_type}_kaiming_uniform_{NUM_EPOCH}eps_width_{width_list[0]}-{width_list[-1]}"
    experiment_dir = PATH_ROOT

    # set training parameters
    root = Path(
        "/ds2/model_zoos/taxonomy/tiny_imagenet/zoos/TinyImageNet/Resnet50_width_v2/tune_zoo_tinyimagenet_Resnet50_width_kaiming_uniform_100eps_width_4-256"
    )

    # load configs to dataframe
    config_df = compile_configs(root)

    # slice config_df for width 2
    config_df = config_df[config_df["model::width"].isin(width_list)]
