...
from ptmz.model_averaging.average_model_epochs_experiment import (
    ModelAverageEpochsTrainable,
)
...
batchsize_list = [1024, 512, 256, 128, 64, 32, 16, 8]
PATH_ROOT = Path("/ds2/model_zoos/taxonomy/cifar_10/zoos/Cifar-10/Resnet18_width_v2")
...
def main():
    ...
    # experiment name
    experiment_name = f"model_average_epochs_cifar-10_{model_type}_kaiming_uniform_{NUM_EPOCH}eps_width_{width_list[0]}-{width_list[-1]}"
    ...
    root = Path(
        "/ds2/model_zoos/taxonomy/cifar_10/zoos/Cifar-10/Resnet18_width_v2/tune_zoo_cifar-10_Resnet18_width_kaiming_uniform_150eps_width_2-256"
    )

    # load configs to dataframe
    config_df = compile_configs(root)

    # slice config_df for batchsize in batchsize_list
    config_df = config_df[config_df["training::batchsize"].isin(batchsize_list)]
    ...
    config = {
        ...
        "epoch_list": [NUM_EPOCH - idx for idx in [4, 3, 2, 1, 0]],
        "align_models": False,
    ...
    experiment = ray.tune.Experiment(
        ...
        run=ModelAverageEpochsTrainable,
        stop={
