...
parser.add_argument(
    "--path-root", type=str, default="/ds2/model_zoos/taxonomy/cifar_100/", help=""
)
parser.add_argument(
    "--data-path",
    type=str,
    default="/ds2/model_zoos/taxonomy/cifar_100/",
    help="",
)
...
def main():
    ...
    # experiment name
    project = f"Cifar-100 {model_type} kaiming-uniform_{NUM_EPOCH}eps_width_{width_list[0]}-{width_list[-1]}"
    experiment_name = f"tune_zoo_cifar-100_{model_type}_kaiming_uniform_{NUM_EPOCH}eps_width_{width_list[0]}-{width_list[-1]}"
    ...
    net_dir = PATH_ROOT.joinpath(f"zoos/Cifar-100/{config['model::type']}_v2")
    try:
